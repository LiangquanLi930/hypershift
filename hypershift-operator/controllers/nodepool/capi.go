...
// Delete handles all deletion logic for a NodePool
func (c *CAPI) Delete2(ctx context.Context) error {
    md := c.machineDeployment()
    ms := c.machineSet()
    mhc := c.machineHealthCheck()
    machineTemplates, err := c.listMachineTemplates()
    if err != nil {
        return fmt.Errorf("failed to list MachineTemplates: %w", err)
    }
    for k := range machineTemplates {
        if err := c.Client.Delete(ctx, machineTemplates[k]); err != nil && !apierrors.IsNotFound(err) {
    ...
    err = c.Client.DeleteAllOf(ctx, &corev1.ConfigMap{},
        client.InNamespace(c.controlplaneNamespace),
        client.MatchingLabels{nodePoolAnnotation: c.nodePool.GetName()},
    )
    if err != nil {
    ...
    if err = c.ensureMachineDeletion(ctx); err != nil {
        return err
    }

    // Delete all secrets related to the NodePool
    if err = c.deleteNodePoolSecrets(ctx); err != nil {
...
func (c *CAPI) ensureMachineDeletion(ctx context.Context) error {
    machines, err := c.getMachinesForNodePool(ctx)
    if err != nil {
        return fmt.Errorf("error getting Machines: %w", err)
    }

    if len(machines) > 0 {
        return fmt.Errorf("there are still Machines in for NodePool %q", c.nodePool.Name)
    }
...
func (c *CAPI) getMachinesForNodePool(ctx context.Context) ([]*capiv1.Machine, error) {
    npAnnotation := client.ObjectKeyFromObject(c.nodePool).String()
    machines := capiv1.MachineList{}

    if err := c.Client.List(ctx, &machines, &client.ListOptions{Namespace: c.controlplaneNamespace}); err != nil {
...
// deleteNodePoolSecrets deletes all secrets related to the NodePool
func (c *CAPI) deleteNodePoolSecrets(ctx context.Context) error {
    secrets := &corev1.SecretList{}
    if err := c.Client.List(ctx, secrets, client.InNamespace(c.controlplaneNamespace), client.MatchingLabels{nodePoolAnnotation: c.nodePool.GetName()}); err != nil {
